...
class GaussianProcess:
    def __init__(self, sq_f, l, sq_n, lr):
        ...
        self.lr = lr
        self.lr_decay = 200
        self.optimizer = torch.optim.Adam([self.sq_n,self.sq_f,self.l],lr=lr)
    ...
    def square_exponential_kernel(self, dist):
        if len(dist) == len(dist[0]):
            return self.sq_f*np.exp(-0.5 * (1 / self.l**2) * dist)+np.eye(len(dist))*self.sq_n
        else:
    ...
    def learn(self,iter):
        ...
        for i in range(iter):
            ...
            cov = self.sq_f*torch.exp(-1/(2*self.l**2)*sqr_dist_mat)+I*self.sq_n
            if not isPSD(cov):
...
print("y len: ",len(y))
# sq_f,l,sq_n,lr
model = GaussianProcess(3.0,12.0,0.45,0.001)
print("now fitting model...")
#model.fit(X,y)
#pickle.dump(model.dist_mat,output)
model.X = X
model.y = y
model.dist_mat = np.array(pickle.load(inputF))[:,:]
inputF.close()
